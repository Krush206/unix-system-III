...
"set", "adefhkmprstwx", "iffiiffffcfcf", 0, cmdset,
"write", "", "", 0, cmdwrite,
"?", "", "", 0, cmdcmd,
NULL,
};
...
/* get arguments for command */
	...
	if( cp->flags&POINTF || cp->flags&PIVOTF ) {
		pt = pts[px^=01]; /* px is an alternate action switch, values 0,1 */
		pis[px] = 0;
	}
...
/* get defined area */
	if( cp->flags&AREAF && !(OPF('l')||OPF('u')||OPF('v')) ) {
		...
		if( !window(pt,pi,ar) ) { /* single point window */
			...
			pt[0] = ar[0] = pt[0]-offset;
			pt[1] = ar[1] = pt[1]-offset;
			pt[2] = ar[2] = pt[0]+2*offset;
			pt[3] = ar[3] = pt[1]+2*offset;
			pi=2; pis[px]=1;
		}
...
readhelp(x,y) /* get point for cmdread */
int *x, *y;
{
	char ch;

	while( getpoint(x,y,&ch), (ch=='$' && !getlabpt(x,y,pis[px]+1)) ) ;
}
