	...
	*cp1 = '\0';
	...
	if( (fp=fopen(cp,"r"))!=NULL ) {
		if( (c=getc(fp))==FCH ) { /* null comment */
			...
			char *p;
			for( p=TEXTBUF,*p++=c; p<(char *)dbuf+BUFSZ && (*p=getc(fp))!=EOF; p++) ;
			*p = '\0'; /* unused part of dbuf buffers text */
			readhelp(pts,pts+1); /* get help from cmdproc */
...
char text[], args[], *table;
{
	...
	if( (fp=fopen(cp,"w"))!=NULL ) {
		...
		if ((i=fwrite((char *)dbuf,sizeof(char),j,fp)) != j) {
...
char text[], args[], *table;
{
	...
	if(OP('u')) {
		...
		setwindow(ar,2);
	} else if( OP('r')>0 && OP('r')<25 ) { /* valid region */
		region(OP('r'),&lx,&ly);
		ar[0] = (int)lx;
		ar[1] = (int)ly;
		ar[2] = (int)lx+XDIM;
...
char text[], args[], *table;
{
	...
	window(pts,pi,&ar);
	scalearea(px=pts[2*pi],py=pts[2*pi+1],&ar,f=OP('f')/100.,OP('k'),OP('e'),table);
	f -= 1;
