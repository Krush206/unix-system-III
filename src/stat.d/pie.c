		...
		if (*(cp=av[0]) == '-' && *++cp != '\0')  {
			while (*cp != '\0')
				switch (*cp++)  {
					case 'p':
							switch (*cp++)  {
								case 'p':	SETINT(op.perpie,cp); break;
...
struct gslparm *gp;
{
	...
	for (j=0,x0=0,y0=RADIUS; j < nsect; j++)  {
		...
		x0 = x3 = RADIUS * cos(PI * slice[j]/total + RAD(x2/RADIUS,y2/RADIUS));
		y0 = y3 = RADIUS * sin(PI * slice[j]/total + RAD(x2/RADIUS,y2/RADIUS));
		if ( keys[j].e )  {
			xoff = x2/5;  yoff = y2/5;
			x1 += xoff;  y1 += yoff;
			x2 += xoff;  y2 += yoff;
		...
		if ( !keys[j].i )  {
			char s[MAXLABEL];
			startarcs(x1,y1,0,keys[j].hue,weight,SOLID+keys[j].f);
			arc(x2,y2);
			arc(x3,y3);
			endarcs();
			startlines(x1,y1,0,keys[j].hue,weight,SOLID+keys[j].f);
			line(xoff,yoff);
			line(x3,y3);
			endlines();
			if (!op.noval)  {
